...
{
  ros::init(argc, argv, "interface_demo");
  ...
  for (std::string target : namedTargets) {
    ...
    if (!move_group.plan(my_plan)) {
      ROS_WARN("[motoman_sia5d_interface_demo/interface_demo] Planning failed. Shutting Down.");
      ros::shutdown();
      return 0;
    }
    ROS_INFO("[motoman_sia5d_interface_demo/interface_demo] Planning OK. Proceeding.");
